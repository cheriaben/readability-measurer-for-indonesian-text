...
open(CEK, "C:/Users/Cheria/Documents/NetBeansProjects/TesKeterbacaan/KataDasar.data") or die $!;
my @katadasar=();
my $status = 0;
while(<CEK>){
chomp;
...
sub stemmingAwalanPe() {
    ...
    $temp = $word;
    $temp =~ s/^pe(m|ng|ny)([aiueo])(.)/$1$2$3/i;
    if($temp !~ $word) {
        return $temp;
    }
    
    $temp = $word;
    $temp =~ s/^pe(n)([aiueo])(.)/t$2$3/i;
    if($temp !~ $word) {
        return $temp;
    }
